function articlesGridInit() {
    ...
    function init_articles() {
        ...
        if (categories) {
            jQuery.each(categories, function (key, value) {
                ...
                jQuery(
                    "#article-filter-form input[type=checkbox][value=" + value + "]"
                ).attr("checked", true);
                jQuery("#article-filter-form input[type=checkbox][value=" + value + "]")
                    .parent()
                    .addClass("active");
                jQuery("#article-filter-form div[data-fake-id=" + value + "]")
                    .parent()
                    .addClass("active");
            });
        }
     ...
     * See line 104
     */
    jQuery(".checkbox__wrap input[type=checkbox]")
        .off("change")
        .on("change", function () {
            var otherFilter = jQuery(this).val();
            var activeCheckbox = jQuery(
                "#article-filter-form input[type=checkbox][value=" + otherFilter + "]"
            );
    ...
    jQuery("img.avatar").on("error", function () {
        jQuery(this)
            .unbind("error")
            .attr("src", "/css/i/broken/avatar_128x128.png");
    });

    console.log("articlesGridInit()");
}